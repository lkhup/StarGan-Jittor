...
class Solver(object):
    ...
    def restore_model(self, resume_iters): #加载模型
    ...
    def build_tensorboard(self): #构建 TensorBoard 可视化日志记录工具的辅助函数
        """Build a tensorboard logger."""
        from logger import Logger
        self.logger = Logger(self.log_dir)

    def update_lr(self, g_lr, d_lr): #手动更新生成器（G）和判别器（D）的学习率
    ...
    def reset_grad(self): #清除生成器（G）和判别器（D）优化器中的梯度信息
        """Reset the gradient buffers."""
        self.g_optimizer.zero_grad()
        self.d_optimizer.zero_grad()

    def denorm(self, x): #将图像像素值的范围从 [-1, 1] 映射回 [0, 1]
        """Convert the range from [-1, 1] to [0, 1]."""
        out = (x + 1) / 2
        return out.clamp_(0, 1)

    def gradient_penalty(self, y, x): #计算梯度惩罚
    ...
    def label2onehot(self, labels, dim): #这个函数将标签整数（如类别 0, 1, 2, ...）转换成 one-hot 编码。
    ...
    def create_labels(self, c_org, c_dim=5, dataset='CelebA', selected_attrs=None): #构造目标属性标签
    ...
    def classification_loss(self, logit, target, dataset='CelebA'): #用于约束生成图像在属性（如头发颜色、性别、年龄等）上的正确性
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i + 1) % self.log_step == 0:
                ...
                # 写入日志文件
                with open('train_log_pytorch.txt', 'a') as f:
                    f.write(f"{log}\n")
